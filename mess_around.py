# Objectives
# define the classes that have specific responsibility in your program
# class named word, attributes "word", length and score
...
# class wordgenerator
#     pass
# Only refactoring the logic code
# Mainloop can remain the same
...
class RackValidator:
    """Scrabble racks have the following properties
    Attributes:
    letters: Scrabble only allowes letter, no numbers or special characters
    length: Cannot contain more than 7 tiles at a time
    """
    def build_rack(self):
        while True:
...
def main():
    ...
    options = Word(list_m)
    name = options.names(list_m)
    s_dict, score =options.scores(list_m)
    length =options.lengths(list_m)
    order = Organize()
